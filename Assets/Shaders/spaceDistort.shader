Shader "Custom/Distort"
{
    Properties
    {
        ...
        _StrengthFilter("Strength Filter", 2D) = "white" {} // Using a strength filter gets rid of square edges on the distorted effect
        _Strength("Strength", float) = 1.0 // Total strength of the effect
        _Speed("Speed", float) = 1.0 // How fast the distortion moves
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            vertexOutput vert(vertexInput input)
            {
                ...
                // unity matrix p = projection matrix
                // unity matrix mv = model view matrix
                // Projected into screenspace
                pos = mul(UNITY_MATRIX_P, 
                      mul(UNITY_MATRIX_MV, float4(0, 0, 0, 1))
                          + float4(pos.x, pos.z, 0, 0));

                // Set the output position to the screenspace position
                output.pos = pos;                
